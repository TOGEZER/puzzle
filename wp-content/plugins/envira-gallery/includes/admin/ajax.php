...
function envira_gallery_sort_gallery(){
	...
	$gallery_data = $common->sort_gallery( $data, $order, $direction );

	// Update the slider data.
	update_post_meta( $post_id, '_eg_gallery_data', $gallery_data );

	// Run hook before finishing.
	do_action( 'envira_ajax_sort_slides', $post_id, $gallery_data, $order );
	...
	foreach ( ( array ) $gallery_data['gallery'] as $id => $data ) {
		...
		$html .= Envira_Gallery_Metaboxes::get_instance()->get_gallery_item( $id, $data, ( ! empty( $data['type'] ) ? $data['type'] : 'image' ), $post_id );
...
function envira_gallery_ajax_remove_image() {
	...
	if ( $image_delete ) {
		...
		if ( ( $attachment->post_parent == $post_id || in_array( $attach_id, $in_gallery_temp ) ) && ( count( $has_gallery ) == 0 ) ) {
